...
| 10 | **DUAL FORMAL REVIEW (Joe's ruling, 7/18 ~09:45, amends §3.8/§9.3)** | Two standing graders, not one: **Sol (gpt-5.6-sol, outside)** and **Luna (claude-fable-5, inside)** both file formal analysis runs on the same pinned revisions and rubric dimensions. The drafting author (fable) still never gates its own work (§9.2 stands). Conflicting findings are resolved at the table — Joe and Luna talk it through, **Joe's word is final** — and the ruling is recorded via decision_create with the dissent trail intact (§3.13 machinery). Score comparability: same dimensions, model recorded per run, grade_history shows both trajectories. §3.8's committee warning stands for anything beyond these two: no third standing grader. |
| 9 | URL-key transport (ChatGPT connectors) | ChatGPT's connector form supports no custom headers, so `/mcp?key=` is accepted — HARD-CAPPED to editor authority regardless of the key's role (URL transport can never write), our access logs disabled, per-client keys rotatable via one Railway var edit. Residual risk: upstream edge logs (Railway) could see the URL; accepted 7/18 with the cap, editor-only blast radius, and rotation posture. Proper future fix if warranted: minimal OAuth token endpoint. |
